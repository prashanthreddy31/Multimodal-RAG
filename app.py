...
class MultimodalRAGSystem:
    ...
    def __init__(self,groq_api_key:str, persist_directory:str ="./chroma_db_v2",
                 main_model:str = "openai/gpt-oss-120b", vision_model:str = "meta-llama/llama-4-scout-17b-16e-instruct",
                 embedding_model:str = "BAAI/bge-m3", temperature:float = 0.1,
                  max_tokens: int =8192 ):
        """
        Initialize the Multimodal RAG system
        
        Args:
            groq_api_key: Groq API key for Gemini
        ...
        #Enhanced text splitter with Chunking Strategy
    ...
    def _extract_text_from_image(self, image_data: bytes) -> str:
        """Extract text content from images using VLM OCR capabilities."""
        try:
            image_b64 = base64.b64encode(image_data).decode()
    ...
    def create_summaries_for_retrieval(self, elements: List[Dict]) -> List[str]:
        """
        Create optimized summaries for better retrieval using LLM.
    ...
    def build_enhanced_vector_database(self, file_paths: List[str], image_paths: List[str]):
        ...
        # Parse documnets using basic extraction
        ...
        for text_elem in parsed_elements["texts"]:
            chunks = self.text_splitter.split_documents(text_elem["content"])

            for chunk in chunks:
                all_elements.append({
                    "content":chunk,
                    "metadata":text_elem["metadata"]
                })
    ...
    def generate_advanced_response(self, query: str, context_docs: List[Document]) -> Dict[str, Any]:
        """
        Generate advanced response using LLM with multimodal context.
